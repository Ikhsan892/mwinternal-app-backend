...
export class PengirimanService {
  ...
  async create(createPengirimanDto: CreatePengirimanDto, file: any): Promise<any> {
    ...
    if (!find_data) {
      ...
      pengiriman.nama_pengiriman = createPengirimanDto.nama_pengiriman.toLowerCase();
  ...
  async update(id: number, updatePengirimanDto: UpdatePengirimanDto, file: any): Promise<any> {
    await this.pengirimanService.update({ id }, { nama_pengiriman: updatePengirimanDto.nama_pengiriman, image_path: file ? `/${file.filename}` : null })
    return {
  ...
  async setActive(active: ActivePengirimanDto): Promise<any> {
    ...
    await this.pengirimanService.update({ id: active.id }, { aktif: !find.aktif });
    return {
      status: HttpStatus.CREATED,
